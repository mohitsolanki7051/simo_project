            ...
            <div class="form-section">
                ...
                <div class="form-grid">
                    <div class="form-group">
                        ...
                        <input type="text" class="form-input" name="name" value="{{ old('name', $category->name) }}" placeholder="e.g., LED Lights" required>
                    ...
                    <div class="form-group">
                        ...
                        <input type="number" class="form-input" name="sort_order" value="{{ old('sort_order', $category->sort_order) }}" min="0">
                    ...
                    <div class="form-group full-width">
                        ...
                        <div class="current-image">
                            @if($category->image)
                                <img src="{{ Storage::url($category->image) }}" alt="{{ $category->name }}" class="existing-image">
                                <div class="image-actions">
                                    <span class="current-text">Current Image</span>
                                    <button type="button" class="btn-remove" onclick="removeCurrentImage()">Remove</button>
                                </div>
                    ...
                    <div class="form-group full-width">
                        ...
                        <textarea class="form-textarea" name="description" rows="4" placeholder="Enter category description">{{ old('description', $category->description) }}</textarea>
                    </div>
                </div>
            </div>
            ...
            <div class="form-section">
                ...
                <div class="form-grid">
                    <div class="form-group full-width">
                        ...
                        <input type="text" class="form-input" name="meta_title" value="{{ old('meta_title', $category->meta_title) }}" placeholder="SEO title for search engines">
                    </div>

                    <div class="form-group full-width">
                        <label class="form-label">Meta Description</label>
                        <textarea class="form-textarea" name="meta_description" rows="3" placeholder="SEO description for search engines">{{ old('meta_description', $category->meta_description) }}</textarea>
                        <span class="form-hint">Recommended: 150-160 characters</span>
                    </div>

                    <div class="form-group full-width">
                        <label class="form-label">Meta Keywords</label>
                        <input type="text" class="form-input" name="meta_keywords" value="{{ old('meta_keywords', $category->meta_keywords) }}" placeholder="comma, separated, keywords">
...
@push('styles')
<style>
    .edit-category-container { padding: 30px; max-width: 1000px; margin: 0 auto; }
    ...
    .existing-image { width: 100px; height: 100px; border-radius: 12px; object-fit: cover; border: 3px solid #c6f6d5; margin-bottom: 10px; }
    .image-actions { display: flex; align-items: center; gap: 10px; }
    .current-text { font-size: 13px; color: #718096; font-weight: 600; }
    .btn-remove { padding: 4px 10px; background: #fed7d7; color: #9b2c2c; border: none; border-radius: 6px; font-size: 12px; font-weight: 600; cursor: pointer; transition: all 0.3s; }
    .btn-remove:hover { background: #feb2b2; }

    .image-upload-wrapper { margin-bottom: 15px; }
    .upload-label { display: flex; flex-direction: column; align-items: center; justify-content: center; padding: 20px; border: 3px dashed #cbd5e0; border-radius: 12px; background: #f7fafc; cursor: pointer; transition: all 0.3s; }
...
<script>
    ...
    function removeCurrentImage() {
        ...
        showAlert('Current image will be removed after update', 'success');
    }
