...
<div class="action-bar no-print">
    <div class="action-bar-left">
        <a href="{{ route('admin.expenses.index') }}" class="back-btn">&#8592; Back</a>
        <span class="page-title">Expense Statement</span>
    </div>
...
<div class="page-wrapper" id="reportToPrint">
            ...
            <td class="doc-info">
                ...
                <div class="doc-meta">
                    @if($fromDate || $toDate)
                        ...
                        to 
                        {{ $toDate ? \Carbon\Carbon::parse($toDate)->format('d M Y') : 'End' }}<br>
                    @else
                        <strong>Period:</strong> All Time<br>
                    @endif
                    <strong>As on Date:</strong> {{ now()->format('d M Y') }}<br>
                    <strong>Generated on:</strong> {{ now()->format('h:i A') }}
                </div>
            </td>
        </tr>
    </table>
    ...
    <div class="summary-band">
        <div class="summary-item">
            <div class="summary-label">Total Business Outflow</div>
        ...
        <div class="summary-item" style="text-align: center;">
            <div class="summary-label">Total Voucher Entries</div>
            <div class="summary-value">{{ count($expenses) }} Records</div>
        </div>
        <div class="summary-item" style="text-align: right;">
            <div class="summary-label">Average Expense value</div>
            <div class="summary-value">₹ {{ count($expenses) > 0 ? number_format($totalExpenses / count($expenses), 2) : '0.00' }}</div>
        </div>
    </div>
    ...
    <table class="data-table">
        <thead>
            <tr>
                ...
                <th class="c" style="width: 12%;">Method</th>
        ...
        <tbody>
            ...
            <tr>
                ...
                <td style="font-weight: 700; color: #111827;">{{ $expense->category_name }}</td>
                <td style="font-family: monospace; font-size: 10px;">{{ $expense->reference_no ?? '—' }}</td>
                <td style="font-size:10px; color:#4b5563;">{{ $expense->description ?? '—' }}</td>
                <td class="c">
                    <span class="badge-method">{{ $expense->payment_method === 'Bank' ? 'Bank Transfer' : $expense->payment_method }}</span>
                </td>
                <td class="r" style="font-weight: 700; color: #dc2626;">
        ...
        <tfoot>
            <tr>
                <td colspan="6" style="text-align: right; font-weight: bold;">TOTAL OUTFLOW</td>
                <td class="r" style="font-weight: bold; color: #dc2626;">
...
function printPDF() {
    ...
    html2pdf().set({
        ...
        filename: 'expense-statement-{{ now()->format("Y-m-d") }}.pdf',
