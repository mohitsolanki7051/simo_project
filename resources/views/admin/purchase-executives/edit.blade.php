...
@section('title', 'Edit Purchase Executive - Admin Panel')
@section('header-title', 'Edit Purchase Executive')
...
<div class="create-executive-container">
    ...
    <div class="page-header">
        <div class="header-left">
            <h2 class="page-title">Edit Purchase Executive: {{ $purchaseExecutive->name }}</h2>
        </div>
    ...
    <form action="{{ route('admin.purchase-executives.update', $purchaseExecutive->id) }}" method="POST" id="executiveForm">
        @csrf
        @method('PUT')
        ...
        <div class="form-card">
            ...
            <div class="form-section">
                ...
                <div class="form-grid">
                    <div class="form-group">
                        ...
                        <input type="text" class="form-input" name="name" value="{{ old('name', $purchaseExecutive->name) }}" placeholder="e.g., Rajesh Kumar" required>
                        @error('name')
                    ...
                    <div class="form-group">
                        ...
                        <input type="text" class="form-input" id="phone" name="phone" value="{{ old('phone', $purchaseExecutive->phone) }}" placeholder="e.g., 9876543210" maxlength="10" required oninput="validatePhone(this.value)">
                    ...
                    <div class="form-group">
                        ...
                        <input type="email" class="form-input" id="email" name="email" value="{{ old('email', $purchaseExecutive->email) }}" placeholder="e.g., rajesh@company.com" oninput="validateEmail(this.value)">
                    ...
                    <div class="form-group">
                        ...
                        <input type="date" class="form-input" name="joining_date" value="{{ old('joining_date', $purchaseExecutive->joining_date ? $purchaseExecutive->joining_date->format('Y-m-d') : date('Y-m-d')) }}" required>
                        @error('joining_date')
                            <div class="error-message">{{ $message }}</div>
                        @enderror
                    </div>
                ...
                <div class="form-grid">
                    <div class="form-group">
                        ...
                        <select class="form-select" name="status" required>
                            <option value="active" {{ old('status', $purchaseExecutive->status) == 'active' ? 'selected' : '' }}>Active</option>
                            <option value="inactive" {{ old('status', $purchaseExecutive->status) == 'inactive' ? 'selected' : '' }}>Inactive</option>
                        </select>
                        @error('status')
                            <div class="error-message">{{ $message }}</div>
                        @enderror
                    ...
                    <div class="form-group full-width">
                        ...
                        <textarea class="form-textarea" name="notes" rows="3" placeholder="Any additional notes about the purchase executive">{{ old('notes', $purchaseExecutive->notes) }}</textarea>
                        @error('notes')
                            <div class="error-message">{{ $message }}</div>
                        @enderror
                    </div>
            ...
            <div class="form-actions">
                ...
                <button type="submit" class="btn-primary" id="submitBtn">✓ Update Purchase Executive</button>
            </div>
        </div>
    </form>
</div>
...
<script>
    ...
    function checkPhoneAvailability(phone) {
        const id = '{{ $purchaseExecutive->id }}';
        fetch(`{{ route("admin.purchase-executives.check-phone") }}?phone=${phone}&exclude_id=${id}`)
    ...
    function checkEmailAvailability(email) {
        ...
        const id = '{{ $purchaseExecutive->id }}';
        fetch(`{{ route("admin.purchase-executives.check-email") }}?email=${encodeURIComponent(email)}&exclude_id=${id}`)
            .then(response => response.json())
            .then(data => {
                if (!data.available) {
