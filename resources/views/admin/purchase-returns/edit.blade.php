{{-- resources/views/admin/purchase-returns/edit.blade.php --}}
@extends('layouts.admin')

@section('title', 'Edit Purchase Return - Admin Panel')
@section('header-title', 'Edit Purchase Return')
...
<div class="return-create-container">
    ...
    <div class="page-header">
        <div class="header-left">
            <h2 class="page-title">Edit Purchase Return: <span class="return-num">{{ $return->return_number }}</span></h2>
        </div>
        <div class="header-right">
            <a href="{{ route('admin.purchase-returns.show', $return->_id) }}" class="btn-small btn-secondary">
                ← Back to Return
            </a>
        </div>
    </div>
    ...
    <div class="return-form-wrapper">
        <form id="purchaseReturnForm" class="return-form">
            @csrf
            @method('PUT')
            ...
            <input type="hidden" name="purchase_invoice_id" id="purchaseInvoiceIdInput" value="{{ $return->purchase_invoice_id }}">
            ...
            <div class="form-section">
                <div class="section-header">
                    ...
                    <span class="invoice-badge">Invoice No.: {{ $return->invoice->invoice_number ?? 'N/A' }}</span>
                </div>
                    ...
                    <div id="selectedInvoiceDetails" class="selected-invoice-details">
                        <div class="invoice-header">
                            <div class="invoice-info">
                                <h4 id="invoiceNumberDisplay">{{ $return->invoice->invoice_number ?? 'N/A' }}</h4>
                                <span class="invoice-date" id="invoiceDateDisplay">
                                    {{ $return->invoice ? \Carbon\Carbon::parse($return->invoice->invoice_date)->format('d-m-Y') : 'N/A' }}
                                </span>
                            </div>
                            <div class="party-info">
                                <span class="party-name" id="partyNameDisplay">
                                   {{ $return->party_name ?? 'N/A' }}
                                </span>
                            </div>
                        </div>

                        <div class="warehouse-badge">
                            <span>🏭 <span id="warehouseNameDisplay">{{ $return->warehouse->name ?? 'N/A' }}</span></span>
                        </div>
                                ...
                                <table class="items-table">
                                    <thead>
                                        <tr>
                                            ...
                                            <th class="th-purchased-qty text-c">Purchased Qty</th>
                            ...
                            <div class="form-row">
                                ...
                                <div class="form-group col-6">
                                    ...
                                    <select name="reason" class="form-control">
                                        ...
                                        <option value="Quality Issue" {{ $return->reason == 'Quality Issue' ? 'selected' : '' }}>Quality Issue</option>
                                        <option value="Expired Product" {{ $return->reason == 'Expired Product' ? 'selected' : '' }}>Expired Product</option>
                        ...
                        <div class="form-actions">
                            <a href="{{ route('admin.purchase-returns.show', $return->_id) }}" class="btn-cancel">
                                Cancel
                            </a>
...
function loadInvoiceDetails() {
    const invoiceId = $('#purchaseInvoiceIdInput').val();

    $.get(`/admin/purchase-returns/invoice-details/${invoiceId}`, function(response) {
        if (response.success) {
...
function renderItemsTable() {
    ...
    // Get current return quantities from PHP
    const currentReturnQtys = {};
    @foreach($return->items as $item)
        currentReturnQtys['{{ $item->purchase_invoice_item_id }}'] = {{ $item->quantity }};
    @endforeach
    ...
    invoiceItems.forEach((item, index) => {
        ...
        const currentQty = currentReturnQtys[item.id] ?? 0;
        ...
        if (isGstInvoice && taxPercent > 0) {
            ...
            taxHtml = `<div class="tax-info"><span class="tax-badge">Non-GST</span></div>`;
        }

        const isChecked = returnQty > 0 ? 'checked' : '';
        const row = `
            <tr data-index="${index}">
                <td class="text-c">
                    <input type="checkbox" class="item-checkbox" ${isChecked} onchange="updateItemSelection(${index})">
                </td>
                <td>
                    <div class="item-name">${item.product_name}</div>
                    ${item.variant_name ? `<div class="item-variant">${item.variant_name}</div>` : ''}
                </td>
                <td class="text-c">${purchasedQty}</td>
                ...
                <td>
                    <div class="qty-control">
                        ...
                        <span class="max-hint">Max: ${remaining}</span>
                    </div>
                    ${taxHtml}
                </td>
...
function updateItemSelection(index) {
    const checkbox = $(`tr[data-index="${index}"] .item-checkbox`);
    const input = $(`tr[data-index="${index}"] .return-qty-input`);

    if (checkbox.prop('checked')) {
        input.prop('disabled', false);
        const maxQty = parseFloat(input.attr('max'));
        input.val(maxQty);
        updateItemQty(index, maxQty);
    } else {
...
$('#purchaseReturnForm').submit(function(e) {
    ...
    $.ajax({
        url: '{{ route('admin.purchase-returns.update', $return->_id) }}',
        type: 'POST',
        data: formData,
        processData: false,
        contentType: false,
        ...
        success: function(response) {
            if (response.success) {
                ...
                setTimeout(() => {
                    window.location.href = '/admin/purchase-returns/' + response.return_id;
                }, 1500);
            }
        },
        error: function(xhr) {
