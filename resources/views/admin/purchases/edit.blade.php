...
<div class="products-container">
    ...
    <div class="invoice-form-wrapper">
        <form id="purchaseInvoiceForm" class="invoice-form">
            ...
            {{-- invoice_type is LOCKED — cannot be changed during edit --}}
                                ...
                                <div class="section-body">
                                    ...
                                    <div class="form-row">
                                        <div class="form-group col-6">
                                            ...
                                            {{-- LOCKED: Invoice type cannot be changed in edit mode --}}
        ...
        <form id="createPartyForm">
            ...
            <div class="modal-body">
                ...
                <div class="form-section-small">
                    ...
                    <div class="form-group">
                        <label class="form-label">Notes</label>
                        <textarea name="notes" class="form-control" rows="2" placeholder="Any additional notes about this party..."></textarea>
                    </div>
                </div>
            </div>
            <div class="modal-actions">
    ...
    <div class="modal-content modal-lg">
        ...
        <div class="modal-body">
            <div style="display:flex;gap:10px;align-items:center;margin-bottom:12px;">
                ...
                <button type="button" onclick="openProductPageInNewTab()" style="padding:7px 14px;background:#555;color:white;border:none;border-radius:3px;font-size:10px;font-weight:500;cursor:pointer;white-space:nowrap;">
                    ➕ New Product
                </button>
            </div>
...
.items-table-container.cash-mode .th-qtys { width:10%; }
.items-table-container.cash-mode .th-mrp  { width:12%; }
...
.summary-label a.disabled-link { background:#e0e0e0 !important; color:#999 !important; border:1px dashed #ccc !important; }
...
// Invoice type is LOCKED from PHP — cannot be changed
const currentInvType = '{{ $invoice->invoice_type }}';
let allProducts      = [];
let partiesXhr       = null;
...
function loadParties() {
    ...
    tbody.html(`<tr><td colspan="6" style="text-align:center;padding:30px;">
        <div class="loading-spinner"></div><p>Loading...</p></td></tr>`);
    partiesXhr = $.get('{{ route("admin.purchases.parties.list") }}', {
        ...
        tbody.html(response.parties.map(function(p) {
            const ob = p.opening_balance > 0 ? '₹' + parseFloat(p.opening_balance).toFixed(2) : '—';
            return `<tr>
                ...
                <td>${p.email || '-'}</td>
                <td>${ob}</td>
                <td><button type="button" class="btn-select-party-row"
                    onclick="selectParty('${p.id}','${p.party_type}')">Select</button></td>
            </tr>`;
        }).join(''));
    }).fail(function(xhr) {
...
function selectParty(partyId, partyType) {
    ...
    $.get(
        ...
        function(res) {
            ...
            // ✅ SHOW DEBIT NOTE REMAINING AMOUNT (same as create)
            ...
            // ✅ Balance recalculate with debit note
            calculateBalance();
        }
    );
}
...
function renderProductTable(products) {
    ...
    tbody.html(products.map(function(p) {
        return `<tr>
            ...
            <td><input type="number" class="qty-input" min="1" step="1" value="1"
                style="width:60px;padding:3px;border:1px solid #ccc;border-radius:2px;text-align:center;font-size:10px;"></td>
            <td>
...
function renderItemsTable() {
    ...
    if (items.length === 0) {
        tbody.html(`<tr class="empty-row"><td colspan="10">
            <div class="empty-items">
                <div class="empty-icon">📦</div>
...
$(document).ready(function() {
    ...
    // Load existing party info
    loadExistingParty();



    // Render existing items (this also calculates totals)
    renderItemsTable();

    // Set amount paid then recalc balance
    ...
    $('#purchaseInvoiceForm').on('submit', function(e) {
        ...
        // Serialize all form inputs except _method (we handle it separately)
        ...
        fd.append('invoice_type', currentInvType); // always locked value
        ...
        $.ajax({
            ...
            success: function(res) {
                if (res.success) {
                    ...
                    setTimeout(function() {
                        window.location.href = '/admin/purchases/' + res.invoice_id;
                    }, 1500);
                } else {
                    showAlert('Error: ' + res.message, 'error');
                    isSubmitting = false;
