...
<div class="pi-wrap">
    ...
    <div class="pi-filters">
        <div class="pi-filters-header">
            ...
            @if(request()->anyFilled(['invoice_number','vendor_id','payment_status','status','period','date_from','date_to','invoice_type','warehouse_id']))
        ...
        <form method="GET" action="{{ route('admin.purchases.index') }}" id="filterForm">
            <div class="pi-filter-row">
                ...
                <!-- Vendor -->
                <div class="pi-filter-group">
                    <label class="pi-filter-label">Vendor</label>
                    <select name="vendor_id" class="pi-select">
            ...
            <table class="pi-table">
                ...
                <tbody>
                    ...
                    <tr class="pi-tr" data-id="{{ $invoice->id }}">
                        ...
                        <td class="tc-vendor">
                            <div class="td-vendor-name" title="{{ $invoice->vendor->company_name ?? 'N/A' }}">
                                {{ $invoice->vendor->company_name ?? 'N/A' }}
                            </div>
                            @if($invoice->vendor && $invoice->vendor->name)
                                <span class="td-vendor-contact">{{ $invoice->vendor->name }}</span>
                            @endif
                        </td>
                        ...
                        <td class="tc-amount">
                            <div class="amount-display">
                                ...
                                @if($invoice->payment_status === 'paid')
                                    <span class="amt-paid-tag">✓ Paid</span>
                                @elseif($invoice->payment_status === 'partial')
                                    <div class="amt-row">
                                        <span class="amt-label">Paid:</span>
                                        <span class="amt-val amt-green">₹ {{ number_format($invoice->total_paid, 2) }}</span>
                                    </div>
                                    <div class="amt-row amt-due-row">
                                        <span class="amt-label">Due:</span>
                                        <span class="amt-val amt-red">₹ {{ number_format($invoice->balance_amount, 2) }}</span>
                                    </div>
                                @elseif($invoice->payment_status === 'unpaid')
                                    <span class="amt-unpaid">₹ {{ number_format($invoice->grand_total, 2) }} due</span>
                                @endif
                            </div>
                        </td>
...
.pi-btn-generate-confirm:hover {
    background: var(--c-primary-dark);
}

.pi-btn-delete-confirm {
    padding: 7px 16px;
    border: none;
    border-radius: var(--r-sm);
    background: #ef4444;
...
.pi-btn-delete-confirm:hover {
    background: #dc2626;
}

.pi-btn-cancel-confirm {
    padding: 7px 16px;
    border: none;
    border-radius: var(--r-sm);
    background: #dc2626;
...
document.getElementById('confirmGenerate').addEventListener('click', function () {
    ...
    .then(data => {
        closeGenerateModal();
        if (data.success) {
            showAlert('Invoice generated successfully! Stock added to warehouse.', 'success');
...
document.getElementById('confirmCancel').addEventListener('click', function () {
    ...
    .then(data => {
        closeCancelModal();
        if (data.success) {
            showAlert('Invoice cancelled successfully. Stock removed from warehouse.', 'success');
            setTimeout(() => location.reload(), 1500);
        } else {
