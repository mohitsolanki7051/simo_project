...
@section('title', 'Edit Salesman - Admin Panel')
@section('header-title', 'Edit Salesman')
...
<div class="create-salesman-container">
    ...
    <div class="page-header">
        <div class="header-left">
            <h2 class="page-title">Edit Salesman: {{ $salesman->name }}</h2>
        </div>
    ...
    <form action="{{ route('admin.salesmen.update', $salesman->id) }}" method="POST" id="salesmanForm">
        @csrf
        @method('PUT')
        ...
        <div class="form-card">
            ...
            <div class="form-section">
                ...
                <div class="form-grid">
                    <div class="form-group">
                        ...
                        <input type="text" class="form-input" name="name" value="{{ old('name', $salesman->name) }}" placeholder="e.g., Rahul Sharma" required>
                        @error('name')
                    ...
                    <div class="form-group">
                        ...
                        <input type="text" class="form-input" id="phone" name="phone" value="{{ old('phone', $salesman->phone) }}" placeholder="e.g., 9876543210" maxlength="10" required oninput="validatePhone(this.value)">
                    ...
                    <div class="form-group">
                        ...
                        <input type="email" class="form-input" id="email" name="email" value="{{ old('email', $salesman->email) }}" placeholder="e.g., rahul@sales.com" oninput="validateEmail(this.value)">
                    ...
                    <div class="form-group">
                        ...
                        <input type="date" class="form-input" name="joining_date" value="{{ old('joining_date', $salesman->joining_date ? $salesman->joining_date->format('Y-m-d') : date('Y-m-d')) }}" required>
                        @error('joining_date')
                            <div class="error-message">{{ $message }}</div>
                        @enderror
                    </div>
            ...
            <div class="form-section">
                ...
                <div class="form-grid">
                    <div class="form-group">
                        ...
                        <select class="form-select" name="salary_type" id="salaryType" required onchange="toggleSalaryTypeFields()">
                            <option value="fixed" {{ old('salary_type', $salesman->salary_type) == 'fixed' ? 'selected' : '' }}>Fixed Only</option>
                            <option value="commission" {{ old('salary_type', $salesman->salary_type) == 'commission' ? 'selected' : '' }}>Commission Only</option>
                            <option value="both" {{ old('salary_type', $salesman->salary_type) == 'both' ? 'selected' : '' }}>Fixed + Commission</option>
                        </select>
                        @error('salary_type')
                            <div class="error-message">{{ $message }}</div>
                        @enderror
                ...
                <div id="commissionSection" class="salary-section" style="{{ in_array(old('salary_type', $salesman->salary_type), ['commission', 'both']) ? 'display: block;' : 'display: none;' }}">
                    <div class="section-subheader">
                        <h4 class="subsection-title">Commission Settings</h4>
                        <label class="toggle-switch">
                            <input type="checkbox" name="commission_enabled" id="commissionEnabled" value="1" {{ old('commission_enabled', $salesman->commission_enabled) ? 'checked' : '' }} onchange="toggleCommissionFields()">
                    ...
                    <div class="commission-fields" id="commissionFields" style="{{ old('commission_enabled', $salesman->commission_enabled) ? '' : 'display: none;' }}">
                        <div class="form-grid">
                            ...
                            <div class="form-group">
                                ...
                                <input type="number" step="0.01" min="0" max="100" class="form-input" name="dealer_commission_percent" value="{{ old('dealer_commission_percent', $salesman->dealer_commission_percent ?? 0) }}" placeholder="e.g., 1.5">
                            ...
                            <div class="form-group">
                                ...
                                <input type="number" step="0.01" min="0" max="100" class="form-input" name="distributor_commission_percent" value="{{ old('distributor_commission_percent', $salesman->distributor_commission_percent ?? 0) }}" placeholder="e.g., 1">
            ...
            <div class="form-section">
                ...
                <div class="form-grid">
                    ...
                    <div class="form-group full-width">
                        ...
                        <textarea class="form-textarea" name="notes" rows="3" placeholder="Any additional notes about the salesman">{{ old('notes', $salesman->notes) }}</textarea>
                        @error('notes')
                            <div class="error-message">{{ $message }}</div>
                        @enderror
                    </div>
            ...
            <div class="form-actions">
                ...
                <button type="submit" class="btn-primary" id="submitBtn">✓ Update Salesman</button>
            </div>
        </div>
    </form>
</div>
...
<script>
    ...
    function checkPhoneAvailability(phone) {
        ...
        fetch(`{{ route("admin.salesmen.check-phone") }}?phone=${phone}&id=${id}`)
    ...
    function checkEmailAvailability(email) {
        ...
        const id = '{{ $salesman->id }}';
        fetch(`{{ route("admin.salesmen.check-email") }}?email=${encodeURIComponent(email)}&id=${id}`)
            .then(response => response.json())
            .then(data => {
                if (!data.available) {
