...
@section('header-title', 'Edit Vendor')
...
<div class="edit-vendor-container">
    ...
    <div class="page-header">
        <div class="header-left">
            <h2 class="page-title">Edit Vendor: {{ $vendor->company_name }}</h2>
        </div>
                ...
                <div class="tab-content active" id="vendor-info">
                    ...
                    <div class="form-grid">
                        <div class="form-group">
                            ...
                            <input type="text" class="form-input" name="company_name" value="{{ old('company_name', $vendor->company_name) }}" placeholder="e.g., ABC Enterprises" required>
                            @error('company_name')
                        ...
                        <div class="form-group">
                            ...
                            <input type="text" class="form-input" name="name" value="{{ old('name', $vendor->name) }}" placeholder="e.g., Rajesh Kumar">
                            @error('name')
                                <div class="error-message">{{ $message }}</div>
                            @enderror
                        </div>
                        ...
                        <div class="form-group">
                            ...
                            <input type="text" class="form-input" id="phone" name="phone" value="{{ old('phone', $vendor->phone) }}" placeholder="e.g., 9876543210" maxlength="10" required oninput="validatePhone(this.value)">
                        ...
                        <div class="form-group">
                            ...
                            <input type="email" class="form-input" id="email" name="email" value="{{ old('email', $vendor->email) }}" placeholder="e.g., contact@company.com" oninput="validateEmail(this.value)">
                        ...
                        <div class="form-group">
                            ...
                            <input type="text" class="form-input" name="gst_number" value="{{ old('gst_number', $vendor->gst_number) }}" placeholder="e.g., 27ABCDE1234F1Z5" maxlength="15" oninput="this.value = this.value.toUpperCase()">
                            @error('gst_number')
                                <div class="error-message">{{ $message }}</div>
                            @enderror
                        </div>
                        ...
                        <div class="form-group">
                            ...
                            <input type="text" class="form-input" name="pan_number" value="{{ old('pan_number', $vendor->pan_number) }}" placeholder="e.g., ABCDE1234F" maxlength="10" oninput="this.value = this.value.toUpperCase()">
                            @error('pan_number')
                                <div class="error-message">{{ $message }}</div>
                            @enderror
                        </div>
                        ...
                        <div class="form-group">
                            ...
                            <input type="number" step="0.01" min="0" class="form-input" name="opening_balance" value="{{ old('opening_balance', $vendor->opening_balance ?? 0) }}" placeholder="0.00">
                        ...
                        <div class="form-group">
                            ...
                            <input type="date" class="form-input" name="opening_balance_date" value="{{ old('opening_balance_date', $vendor->opening_balance_date ? $vendor->opening_balance_date->format('Y-m-d') : '') }}">
                        ...
                        <div class="form-group">
                            ...
                            <input type="number" step="0.01" min="0" class="form-input" name="credit_limit" value="{{ old('credit_limit', $vendor->credit_limit) }}" placeholder="e.g., 50000">
                        ...
                        <div class="form-group">
                            ...
                            <select class="form-select" name="purchase_executive_id">
                                ...
                                @foreach($purchaseExecutives as $pe)
                                    <option value="{{ $pe->_id }}" {{ old('purchase_executive_id', $vendor->purchase_executive_id) == $pe->_id ? 'selected' : '' }}>
                                        {{ $pe->name }}
                                    </option>
                                @endforeach
                            </select>
                        ...
                        <div class="form-group full-width">
                            ...
                            <textarea class="form-textarea" name="notes" rows="3" placeholder="Any additional notes about the vendor">{{ old('notes', $vendor->notes) }}</textarea>
                            @error('notes')
                                <div class="error-message">{{ $message }}</div>
                            @enderror
                        </div>
                ...
                <div class="tab-content" id="bank-info">
                    ...
                    <div class="form-grid">
                        <div class="form-group">
                            ...
                            <input type="text" class="form-input" name="account_holder_name" value="{{ old('account_holder_name', $vendor->account_holder_name) }}" placeholder="e.g., ABC Enterprises">
                        ...
                        <div class="form-group">
                            ...
                            <input type="text" class="form-input" name="bank_name" value="{{ old('bank_name', $vendor->bank_name) }}" placeholder="e.g., State Bank of India">
                            @error('bank_name')
                                <div class="error-message">{{ $message }}</div>
                            @enderror
                        </div>
                        ...
                        <div class="form-group">
                            ...
                            <input type="text" class="form-input" name="account_number" value="{{ old('account_number', $vendor->account_number) }}" placeholder="e.g., 12345678901">
                        ...
                        <div class="form-group">
                            ...
                            <input type="text" class="form-input" name="ifsc_code" value="{{ old('ifsc_code', $vendor->ifsc_code) }}" placeholder="e.g., SBIN0001234" oninput="this.value = this.value.toUpperCase()">
                            @error('ifsc_code')
                                <div class="error-message">{{ $message }}</div>
                            @enderror
                        </div>
...
<script>
    ...
    function checkPhoneAvailability(phone) {
        fetch(`/admin/vendors/check-phone?phone=${phone}&exclude_id=${vendorId}`)
    ...
    function checkEmailAvailability(email) {
        ...
        fetch(`/admin/vendors/check-email?email=${encodeURIComponent(email)}&exclude_id=${vendorId}`)
            .then(response => response.json())
            .then(data => {
                if (!data.available) {
    ...
    function loadAddresses() {
        ...
        fetch(`/admin/vendors/${vendorId}/addresses/billing`, {
            headers: { 'X-Requested-With': 'XMLHttpRequest' }
        })
        .then(response => response.json())
        .then(data => {
            if (data.success) {
                addresses.billing = data.addresses;
                renderAddresses('billing', data.addresses);
            }
        });

        // Load shipping addresses
        fetch(`/admin/vendors/${vendorId}/addresses/shipping`, {
            headers: { 'X-Requested-With': 'XMLHttpRequest' }
        })
        .then(response => response.json())
        .then(data => {
            if (data.success) {
                addresses.shipping = data.addresses;
                renderAddresses('shipping', data.addresses);
            }
        });
    }

    function renderAddresses(type, addressesList) {
    ...
    function saveAddress() {
        ...
        const url = addressId
            ? `/admin/vendors/addresses/${addressId}`
    ...
    function deleteAddress(type, addressId) {
        ...
        fetch(`/admin/vendors/addresses/${addressId}`, {
            method: 'DELETE',
            headers: {
                'X-CSRF-TOKEN': '{{ csrf_token() }}',
                'X-Requested-With': 'XMLHttpRequest'
    ...
    function setDefaultAddress(type, addressId) {
        fetch(`/admin/vendors/addresses/${addressId}/default`, {
            method: 'POST',
            headers: {
                'X-CSRF-TOKEN': '{{ csrf_token() }}',
                'X-Requested-With': 'XMLHttpRequest'
    ...
    function copyBillingToShipping() {
        ...
        // Delete all existing shipping addresses first
        const deletePromises = addresses.shipping.map(addr => {
            const addrId = addr._id || addr.id;
            return fetch(`/admin/vendors/addresses/${addrId}`, {
                method: 'DELETE',
                headers: {
                    'X-CSRF-TOKEN': '{{ csrf_token() }}',
                    'X-Requested-With': 'XMLHttpRequest'
        ...
        Promise.all(deletePromises)
            .then(() => {
                // Then copy billing addresses to shipping
                const createPromises = addresses.billing.map(addr => {
                    ...
                    return fetch(`/admin/vendors/${vendorId}/addresses`, {
                        method: 'POST',
                        headers: {
                            'Content-Type': 'application/json',
                            'X-CSRF-TOKEN': '{{ csrf_token() }}',
